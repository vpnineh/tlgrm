...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    function getConfigItems($type, $html)
{
    // استخراج کانفیگ‌ها به همراه هش‌تگ (بخش بعد از #)
    preg_match_all('/(' . $type . ':\/\/[^\s"\'>]+)(#[^\s"\'>]+)?/i', $html, $matches);

    $results = [];
    if (!empty($matches[1])) {
        foreach ($matches[1] as $index => $baseConfig) {
            $tag = $matches[2][$index] ?? '';
            $results[] = $baseConfig . $tag;
        }
    }
    return $results;
}
...
function saveConfigsByCountry(array $configs)
{
    $flagRegex = '/[\x{1F1E6}-\x{1F1FF}]{2}/u'; // Unicode flag emojis
    $countryMap = [];
    ...
    foreach ($configs as $type => $items) {
        foreach ($items as $config) {
            ...
            if (count($parts) < 2) continue; // اگر هش‌تگ نداشت رد کن

            $tag = $parts[1];
            if (preg_match_all($flagRegex, $tag, $matches)) {
    ...
    foreach ($countryMap as $countryCode => $countryConfigs) {
        $filePath = "location/" . strtoupper($countryCode) . ".txt";
        file_put_contents($filePath, implode("\n", $countryConfigs));
        echo "✅ Saved " . count($countryConfigs) . " configs to $filePath\n";
    }
}
...
function emojiToCountryCode($emoji)
{
    ...
    for ($i = 0; $i < $length; $i++) {
        ...
        $codePoint = unpack('N', mb_convert_encoding($char, 'UCS-4BE', 'UTF-8'))[1];
        $codePoints[] = chr($codePoint - 0x1F1E6 + ord('A'));
    }
